...
{
public:
	static NxClothDesc GenCDesc(NxScene* scene,NxPhysicsSDK* gPhysicsSDK,PolyhedronNode* Pn)
	{
		...
		return clothDesc;
		//mCloth = scene->createCloth(clothDesc);
	}
	...
	static void genPolyMeshDesc(NxClothMeshDesc &desc,PolyhedronNode* Pn)
	{
		...
		*CxP = Pn->P->Mesh ;
		...
		{
			Point_3 p = i->point();
			((NxVec3*)desc.points)	[j] = NxVec3(p.x() , p.y() , p.z() );
		}
