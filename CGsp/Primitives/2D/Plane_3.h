	...
	Polyhedron Draw()
	{
		...
		if( width_Seg >= 2 )
		{
			...
			for ( int i = 1 ; i < width_Seg ; i++ )
			{
				...
				h11->vertex()->point() = Point ( w*i, 0, 0 );
				h22->vertex()->point() = Point ( w*i, length, 0 );
		...
		if( length_Seg >= 2 )
		{
			...
			for ( int i = 1 ; i <= length_Seg ; i++ )
			{
				...
				h33->vertex()->point() = Point ( width, l*i, 0 );
				h44->vertex()->point() = Point ( 0, l*i, 0 );
				...
				for ( int j = 1 ; j < width_Seg ; j++ )
				{
					...
					a->vertex()->point() = Point ( w*j, l*i ,0 );
